self.JSDON = (function (exports) {
    ...
    while (i < length) {
      ...
      switch (nodeType) {
        case ELEMENT_NODE:
          ...
          for (var j = 0; j < _length; j++) parentNode.setAttribute(attributes[j].name, attributes[j].value);
          skipCheck = true;
          break;
        case TEXT_NODE:
        ...
        case DOCUMENT_NODE:
          ...
          if (array[i] === DOCUMENT_TYPE_NODE) {
            ...
            var _name = array[i++];
            var args = [_name];
            while (typeof array[i] === 'string') args.push("\"".concat(array[i++], "\""));
            switch (args.length) {
              case 2:
  ...
  var pushElement = function pushElement(_ref2, output, filter) {
    ...
    for (var i = 0, length = attributes.length; i < length; i++) pushAttribute(attributes[i], output);
    for (var _i = 0, _length = childNodes.length; _i < _length; _i++) pushNode(childNodes[_i], output, filter);
    mergeClosing(output);
  };
